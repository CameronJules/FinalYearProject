...
url = "https://5vawmc8ccc.execute-api.eu-west-1.amazonaws.com/dev/time-stretch"
headers = {
...
def generate_page(url, headers):
    st.set_page_config(page_title="Time Stretch Demo")

    st.title("Time Stretch Demo")

    st.text(
        '''
            This endpoint is similar to the pitch shift, it provides a analogus service.
            The main point behind this is to allow for these light weight functions to
            be chained together.
        '''
    )
    ...
    with st.container(border=True):
        ...
        if stretch_amount != 0:
            url = url+f"?stretch_amount={stretch_amount}"
