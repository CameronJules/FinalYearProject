...
url = "https://5vawmc8ccc.execute-api.eu-west-1.amazonaws.com/dev/instrument-detector"
headers = {
...
def generate_page(url, headers):
    ...
    st.title("Instrument Detection Demo")


    # File upload
    with st.container(border=True):
        uploaded_file = st.file_uploader(label="Upload MP3 file here", type="mp3")
        top_n = st.number_input("Insert number of instruments to return (Top n instruments)",step=1)
        if top_n > 0:
